    ...
    for (dirpath, dirnames, filenames) in os.walk(os.path.join(ccg_data_path,
        ...
        for filename in filenames:
            ...
            if (filename.endswith('.pdf')):
                print(str(datetime.now()) + ': Processing: ' + str(filename))
                if os.path.exists(
...
def parse_wrapper(ccg_data_path, filepath, abrev):
    try:
        ...
        df.to_csv(os.path.join(filepath, 'merged',
                               abrev + '.csv'), index=False)
    except Exception as e:
        print('Parsing files failed: ' + traceback.format_exc())
        print('test')
...
def parse_data(filepath, department, filestoskip=[]):
    ...
    for file_ in allFiles:
        print(str(datetime.now()) + ': Working on ' + file_.split('\\')[-1])
